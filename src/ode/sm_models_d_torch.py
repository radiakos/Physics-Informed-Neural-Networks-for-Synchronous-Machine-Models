...
class SynchronousMachineModelsTorch():
    def __init__(self, t, num_of_points, machine_params, system_params, model_flag):
        ...
        Attributes:
            ...
            R_e (float): The exciter resistance (only if AVR exists).
            ...
            $X_d$ & reactance of d-axis(pu)  \\ \hline
            write the avr and governor parameters in that way and also put units
            $K_A$ & AVR gain(pu)  \\ \hline
            $T_A$ & AVR time constant(s)  \\ \hline
            $K_E$ & AVR excitation system gain(pu)  \\ \hline
            $T_E$ & AVR excitation system time constant(s)  \\ \hline
            $K_F$ & AVR feedback gain(pu)  \\ \hline
            $T_F$ & AVR feedback time constant(s)  \\ \hline
            $V_{ref}$ & Reference voltage(pu)  \\ \hline
            $P_c$ & Governor control signal(pu)  \\ \hline
            $R_d$ & Governor droop coefficient(pu)  \\ \hline
            $T_{ch}$ & Governor time constant(s)  \\ \hline
            $T_{sv}$ & Governor servo time constant(s)  \\ \hline
    ...
    def calculate_voltages(self, theta, I_d, I_q):
        ...
        V_t = torch.sqrt(V_d ** 2 + V_q ** 2)  # equal to Vs
        return V_t
    ...
    def solve(self, x0, method):
        ...
        - method: boolean flag to determine the method to solve the differential equations
